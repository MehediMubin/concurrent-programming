...
func saverWorker(id int, saveQueue <- chan ImageJob, results chan <- Result, wg *sync.WaitGroup) {
	...
	for job := range saveQueue {
		log.Printf("[Saver %d] Saving %s", id, job)
		err := saveImage(job.FileName)
		if err != nil {
			results <- Result{Job: job, Err: err}
		} else {
...
func main() {
	imageFiles := []string{
		"wedding.jpg", "birthday.jpg", "vacation.jpg", "nature1.jpg", "nature2.jpg", "fashion.jpg",
	}
	...
	var processorWg, saverWg sync.WaitGroup
	for i := range numProcessor {
		processorWg.Add(1)
		go processorWorker(i, jobQueue, saveQueue, results, &processorWg)
	}

	for i := range numSaver {
		saverWg.Add(1)
